...
| Hermes Agent | `hermes` | none | Self-improving autonomous agent (`/root/hermes/`, model `nousresearch/hermes-4-70b` via OpenRouter, bot `@StraitsHermesBot`, owner-only Telegram polling). Confined sandbox alongside OpenClaw: non-root, `read_only` rootfs, `cap_drop: ALL`, `no-new-privileges`; networks `hermes_egress` + `hermes_db` (internal). Read/write scratch in `/research/hermes` + `/docs/hermes`; parent trees read-only. `hermes_ro` Postgres role (24-table SELECT allowlist). env_file `/root/secrets/hermes.env`. See Part 7 of ROADMAP. |
