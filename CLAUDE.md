...
| PostgreSQL | `root-portfolio_postgres-1` | 5432 | Two databases on the same instance: `portfolio` (portfolio intelligence: 31 tables, `price_history`, `portfolio_positions`, `fx_rates`) and `affection` (affection bot: `affection_conversation`, `affection_outbox` — isolated from financial data since 2026-06-30). Internal only — not exposed externally. |
...
| OpenClaw | `openclaw` | 18789 (loopback only) | Sandboxed AI assistant (`/root/openclaw/`, model `openai/gpt-5.4-mini`, bot `@StraitsClawBot`, owner-only Telegram polling). Hardened: non-root, `read_only` rootfs, `cap_drop: ALL`, `no-new-privileges`; networks `openclaw_egress` + `openclaw_db` (internal, never `root_default`/`dind`). Read-only mounts `/research`, `/config` + writable `/workspace`; `openclaw_ro` Postgres role (24-table SELECT allowlist, no PII/secrets). Control UI on `127.0.0.1:18789` (SSH tunnel). env_file `/root/secrets/openclaw.env`. See Part 6 of ROADMAP. |
| Hermes Agent | `hermes` | none | Self-improving autonomous agent (`/root/hermes/`, model `deepseek/deepseek-v4-pro` via OpenRouter, bot `@StraitsHermesBot`, owner-only Telegram polling). Confined sandbox alongside OpenClaw: non-root, `read_only` rootfs, `cap_drop: ALL`, `no-new-privileges`; networks `hermes_egress` + `hermes_db` (internal). Read/write scratch in `/research/hermes` + `/docs/hermes`; parent trees read-only. `hermes_ro` Postgres role (24-table SELECT allowlist). env_file `/root/secrets/hermes.env`. Communication contract (outbound feedback channel + inbound nudge inbox) in `docs/HERMES-PROTOCOL.md`. See Part 7 of ROADMAP. |
...
| System Metrics Collector (host) | — | — | `scripts/system-metrics-collector.py` + `system-metrics.timer` (every 30min). Writes VPS host metrics (disk, memory, load, Docker, backup) to `/research/system/vps_health.json`. Pure stdlib. Installed 2026-06-30. |
...
| `/root/docs/HERMES-PROTOCOL.md` | Hermes ↔ stack communication contract — outbound feedback channel + inbound nudge inbox (schema, categories, processing procedure) |
