...
class TestSchemaRoundtrip:
    def test_schema_roundtrip_required_keys(self, tmp_inbox):
        ...
        for key in ("schema_version", "nudge_id", "source", "category", "created_at",
                    "urgency", "expires_at", "advisory", "evidence", "subject"):
    ...
    def test_schema_roundtrip_no_evidence(self, tmp_inbox):
        path = write_nudge(category="general", source="claude-code", subject="No evidence", body="body", inbox_dir=tmp_inbox)
        fm, _ = _read_front_matter(path)
        assert fm["evidence"] in ([], None)
        assert fm["advisory"] is True

    def test_schema_roundtrip_category_present(self, tmp_inbox):
        path = write_nudge(category="research-published", source="claude-code", subject="Category roundtrip", body="b", inbox_dir=tmp_inbox)
        fm, _ = _read_front_matter(path)
        assert fm["category"] == "research-published"

    def test_schema_roundtrip_version_is_2(self, tmp_inbox):
        path = write_nudge(category="general", source="claude-code", subject="Version check", body="b", inbox_dir=tmp_inbox)
        fm, _ = _read_front_matter(path)
        assert fm["schema_version"] == 2
...
class TestValidation:
    def test_validation_empty_source(self, tmp_inbox):
        with pytest.raises(ValueError, match="source"):
            write_nudge(category="general", source="", subject="s", body="b", inbox_dir=tmp_inbox)

    def test_validation_bad_source_chars(self, tmp_inbox):
        with pytest.raises(ValueError, match="source"):
            write_nudge(category="general", source="Claude Code!", subject="s", body="b", inbox_dir=tmp_inbox)

    def test_validation_empty_subject(self, tmp_inbox):
        with pytest.raises(ValueError, match="subject"):
            write_nudge(category="general", source="claude-code", subject="", body="b", inbox_dir=tmp_inbox)

    def test_validation_subject_too_long(self, tmp_inbox):
        with pytest.raises(ValueError, match="subject"):
            write_nudge(category="general", source="claude-code", subject="x" * 201, body="b", inbox_dir=tmp_inbox)

    def test_validation_empty_body(self, tmp_inbox):
        with pytest.raises(ValueError, match="body"):
            write_nudge(category="general", source="claude-code", subject="s", body="", inbox_dir=tmp_inbox)

    def test_validation_bad_urgency(self, tmp_inbox):
        with pytest.raises(ValueError, match="urgency"):
            write_nudge(category="general", source="claude-code", subject="s", body="b", urgency="critical", inbox_dir=tmp_inbox)

    def test_validation_malformed_evidence_not_list(self, tmp_inbox):
        with pytest.raises(ValueError, match="evidence"):
            write_nudge(category="general", source="claude-code", subject="s", body="b", evidence="not-a-list", inbox_dir=tmp_inbox)

    def test_validation_malformed_evidence_entry(self, tmp_inbox):
        with pytest.raises(ValueError, match="evidence"):
            write_nudge(category="general", source="claude-code", subject="s", body="b", evidence=[{"type": "plan"}], inbox_dir=tmp_inbox)

    def test_validation_bad_expires_at(self, tmp_inbox):
        with pytest.raises(ValueError, match="expires_at"):
            write_nudge(category="general", source="claude-code", subject="s", body="b", expires_at="not-a-date", inbox_dir=tmp_inbox)

    def test_validation_good_expires_at_accepted(self, tmp_inbox):
        path = write_nudge(category="general", source="claude-code", subject="s", body="b",
                            expires_at="2026-07-03T00:00:00Z", inbox_dir=tmp_inbox)
        fm, _ = _read_front_matter(path)
        assert fm["expires_at"] == "2026-07-03T00:00:00Z"

    def test_validation_missing_category(self, tmp_inbox):
        with pytest.raises(ValueError, match="category"):
            write_nudge(source="claude-code", subject="s", body="b", inbox_dir=tmp_inbox)

    def test_validation_bad_category_chars(self, tmp_inbox):
        with pytest.raises(ValueError, match="category"):
            write_nudge(category="Bad Category!", source="claude-code", subject="s", body="b", inbox_dir=tmp_inbox)


class TestAtomicWrite:
    def test_atomic_no_tmp_remnants(self, tmp_inbox):
        for i in range(5):
            write_nudge(category="general", source="claude-code", subject=f"atomic check {i}", body="b", inbox_dir=tmp_inbox)
    ...
    def test_atomic_full_content_present_immediately(self, tmp_inbox):
        path = write_nudge(category="general", source="claude-code", subject="content check", body="full body text", inbox_dir=tmp_inbox)
...
class TestPermissionsBootstrap:
    def test_bootstrap_chown_and_not_world_writable(self, tmp_inbox):
        write_nudge(category="general", source="claude-code", subject="perm check", body="b", inbox_dir=tmp_inbox)
...
class TestNamingConvention:
    def test_naming_filename_pattern(self, tmp_inbox):
        path = write_nudge(category="general", source="claude-code", subject="WS-1 consumer live!", body="b", inbox_dir=tmp_inbox)
    ...
    def test_naming_nudge_id_matches_filename_stem(self, tmp_inbox):
        path = write_nudge(category="general", source="claude-code", subject="stem check", body="b", inbox_dir=tmp_inbox)
        fm, _ = _read_front_matter(path)
        stem = os.path.splitext(os.path.basename(path))[0]
        assert fm["nudge_id"] == stem

    def test_naming_collision_appends_suffix(self, tmp_inbox):
        path1 = write_nudge(category="general", source="claude-code", subject="collide", body="b1", inbox_dir=tmp_inbox)
        path2 = write_nudge(category="general", source="claude-code", subject="collide", body="b2", inbox_dir=tmp_inbox)
        assert path1 != path2
...
class TestCli:
        ...
        try:
            return subprocess.run(
                [sys.executable, "/root/scripts/nudge-hermes.py",
                 "--source", "claude-code", "--category", "general",
                 "--subject", "cli test", "--body-file", body_file, *extra_args],
    ...
    def test_cli_invalid_urgency_exits_nonzero(self, tmp_inbox):
        result = subprocess.run(
            [sys.executable, "/root/scripts/nudge-hermes.py",
             "--source", "claude-code", "--category", "general",
             "--subject", "bad", "--urgency", "notaurgency",
