...
def _build_message(front_matter: dict, narrative: str) -> str:
    ...
    front_matter must contain:
      ...
      videos: [{title, watch_url, channel_name, summary}]
    narrative: not used for youtube (summaries come from per-video data in front_matter)
    Each video section: [Title](url) — Channel\nSummary text
    """
    ...
    for v in videos:
        ...
        link_line = f"[{title}]({url}) — _{channel}_" if url else f"*{title}* — _{channel}_"
        section = link_line
        if summary:
            section += "\n" + summary
        video_sections.append(section)

    body = ("\n\n" + "—" * 30 + "\n\n").join(video_sections) if video_sections else "(no summaries)"
    return f"{header}\n\n{body}"
